...
function startEdit(id){
  ...
  document.getElementById('specScreen').value = specMap['حجم الشاشة'] || '';
  document.getElementById('specCamera').value = specMap['الكاميرا'] || '';
  document.getElementById('specBattery').value = specMap['البطارية'] || '';
  document.getElementById('specStorage').value = specMap['الذاكرة'] || '';
  document.getElementById('specRam').value = specMap['الرام'] || '';
...
function resetForm(){
  ...
  ['specScreen','specCamera','specBattery','specStorage','specRam'].forEach(id=> document.getElementById(id).value='');
...
let currentImageBase64 = null; // holds the final Cloudinary URL once uploaded

document.getElementById('imageInput').addEventListener('change', async (e)=>{
  const file = e.target.files[0];
  if(!file) return;
  const note = document.getElementById('compressNote');
  note.textContent = 'جارِ ضغط الصورة ورفعها...';
  try{
    const originalKB = Math.round(file.size/1024);
    const { url, sizeKB } = await compressAndUpload(file);
    currentImageBase64 = url;
    const preview = document.getElementById('imgPreview');
    preview.src = url;
    preview.style.display='block';
    document.getElementById('uploadHint').style.display='none';
    note.textContent = `تم رفع الصورة: ${originalKB}KB ← ${sizeKB}KB ✅ (Cloudinary)`;
  }catch(err){
    console.error(err);
    note.textContent = 'تعذر رفع الصورة، تأكد من الاتصال بالإنترنت وحاول مرة ثانية';
  }
});
      ...
      img.onload = ()=>{
        ...
        canvas.toBlob(async (blob)=>{
          ...
          try{
            ...
            resolve({ url, sizeKB: Math.round(blob.size/1024) });
          }catch(err){
            reject(err);
          }
        }, 'image/jpeg', 0.7);
...
document.getElementById('productForm').addEventListener('submit', async (e)=>{
  ...
  const fixedSpecs = [
    { label:'حجم الشاشة', value: document.getElementById('specScreen').value.trim() },
    { label:'الكاميرا', value: document.getElementById('specCamera').value.trim() },
    { label:'البطارية', value: document.getElementById('specBattery').value.trim() },
    { label:'الذاكرة', value: document.getElementById('specStorage').value.trim() },
    { label:'الرام', value: document.getElementById('specRam').value.trim() }
  ];
  const specs = fixedSpecs.filter(s=> s.value);
